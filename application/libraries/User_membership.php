...
class User_membership
{
	 ...
	 */
	function create_user($user_data=array(),$profile_user_data=array())
	{
		$result_user=array();
		$result_user['error']=1;
		...
		if(count($user_data)>0)
		{
			...
			{		
				...
				$data['virtual_address']  = "http://www.leadquestmarketing.com/".$user_data['username']."";
				$data['membership_plan']  = $profile_user_data['membership_plan'];
            	$data['firstname']        = $profile_user_data['firstname'];
            	$data['lastname']         = $profile_user_data['lastname'];
            	$data['emailcontact']     = $profile_user_data['emailcontact'];
            	$data['phone']   	      = $profile_user_data['phone'];
            	$data['facebook']         = $profile_user_data['facebook'];
           	 	$data['skype']            = $profile_user_data['skype'];
            	$data['youtube']          = $profile_user_data['youtube'];
            	

            	
				$date = date_create(date("Y-m-d"));
				//$duration = ($profile_user_data['membership_plan']=="annual")?"1 year":"1 month";
				switch ($profile_user_data['membership_plan']) {
					case 'annual': $duration    = "1 year"; break;
					case 'free': $duration      = "1 year"; break;
					case 'monthly': $duration   = "1 month"; break;
					case 'semestral': $duration = "6 months"; break;
				}
				date_add($date, date_interval_create_from_date_string($duration));

				$data['account_expiration_date']  = date_format($date, 'Y-m-d');
